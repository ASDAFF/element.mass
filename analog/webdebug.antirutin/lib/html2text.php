...
class Html2Text {
	...
	*/
	...
	protected function converter(&$text)
	{
			...
			$text = html_entity_decode($text, $this->htmlFuncFlags, static::ENCODING);
	...
	protected function convertBlockquotes(&$text)
	{
			if (preg_match_all('/<\/*blockquote[^>]*>/i', $text, $matches, PREG_OFFSET_CAPTURE)) {
					...
					foreach ($matches[0] as $m) {
							$m[1] = mb_strlen(mb_substr($originalText, 0, $m[1]));
							if ($m[0][0] == '<' && $m[0][1] == '/') {
									$level--;
									if ($level < 0) {
											...
											$body = preg_replace('/((^|\n)>*)/', '\\1> ', trim($body));
											$body = '<pre>' . htmlspecialchars($body, $this->htmlFuncFlags, static::ENCODING) . '</pre>';
											// Re-set text width
	...
	{
			$str = html_entity_decode($str, $this->htmlFuncFlags, static::ENCODING);
			$str = mb_strtoupper($str);
			$str = htmlspecialchars($str, $this->htmlFuncFlags, static::ENCODING);
...
class Html2Text_cp1251 extends Html2Text {
	const ENCODING = 'CP1251';
}

class Html2Text_utf8 extends Html2Text {
	const ENCODING = 'UTF-8';
}
