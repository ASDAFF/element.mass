...
class CWDA_SetValues extends CWDA_Plugin {
	...
	static function GetMessage($Code, $ConvertCharset=false) {
		$MESS = array(
			...
			'OPTION_KEEP_EXISTS_MULTIPLE_VALUES' => 'Добавить новые значения к уже имеющимся значениям у товара (только для множественных свойств)',
			'OPTION_KEEP_EXISTS_MULTIPLE_VALUES_HINT' => 'Используйте данную опцию для множественных свойство, чтобы не перезатирать значение заново, а чтобы добавлять значения к имеющимся. Например, если в множественном свойстве типа «Список» у Вас отмечено «Пункт1», и записывается значение «Пункт2», то у товара будут сохранены оба значения - «Пункт1» и «Пункт2».<br/><br/>Это может быть очень полезным в случае, если, например, группе товаров нужно добавить одинаковое значение свойства вдобавок к имеющимся, или, например, привязать к группе товаров один и тот же связанный товар, не удаляя ранее привязанные.',
			//
			...
			//
			'USE_FOR' => 'Применить значения для:',
			'USE_FOR_PRODUCT' => 'товара',
			'USE_FOR_OFFERS' => 'предложений',
			'USE_FOR_ALL' => 'товаров и предложений',
		);
		$MESS = trim($MESS[$Code]);
	...
	static function AddHeadData() {
		...
		?>
		<script src="/bitrix/js/aspro.next/customfilter_control.min.js"></script>
		<script>
		...
		function WDA_<?=self::CODE?>_Fill(){
			...
			SelectTarget.find('[value=ID],[value=ACTIVE_DATE],[value=SECTION_GLOBAL_ACTIVE],[value=CATALOG_AVAILABLE],[value=TIMESTAMP_X],[data-type=F],[data-type="S:Video"],[value^=CATALOG_STORE_AMOUNT_]').remove();
			if (IBlock>0) {
			...
			$('#wda_field_target option[value=CATALOG_WEIGHT]').insertBefore('#wda_field_target option[value=CATALOG_LENGTH]').text('<?=self::GetMessage('OPTION_WEIGHT',true);?>');
			// Event handlers
			SelectTarget.on('change',function(){
				...
				var isMultiple = $(this).find('option:selected').parent().is('optgroup[data-group="PROPERTIES"]') && $(this).find('option:selected').text().substr(-2)=='+]';
				if(isMultiple) {
					$('#wda_keep_exists_multiple_values_param_<?=self::CODE?>').show();
				} else {
					$('#wda_keep_exists_multiple_values_param_<?=self::CODE?>').hide();
				}
			});
		}
		function WDA_Params_Update_<?=self::CODE?>(Profile){
			$('#wda_action_params').Deserialize(window.WdaProfileData['~PARAMS']);
		}
		</script>
		<style>
	...
	static function ShowAdditionalSettings() {
		...
		if (is_array($arProperty)) {
			...
			switch($arProperty['PROPERTY_TYPE']) {
				...
				default:
					if (strlen($arProperty['ID'])) {
						...
						if($arProperty['PROPERTY_TYPE'].':'.$arProperty['USER_TYPE'] == 'S:SAsproCustomFilter'){
							$strHtml = preg_replace('#<tr>#', '<tr id="tr_PROPERTY_'.$arProperty['ID'].'">', $strHtml, 1);
						}
						print $strHtml;
					}
			...
			if(preg_match('#^CATALOG_(.*?)$#', $Target) && CModule::includeModule('catalog')){
				$arCatalog = CCatalog::GetById($IBlockID);
				?>
				<?if($arCatalog['OFFERS_IBLOCK_ID']):?>
					<div><br/></div>
					<div style="padding-bottom:4px;"><?=self::GetMessage('USE_FOR',true);?></div>
					<div>
						<select name="params[use_for]">
							<option value="product"><?=self::GetMessage('USE_FOR_PRODUCT',true);?></option>
							<option value="offers"><?=self::GetMessage('USE_FOR_OFFERS',true);?></option>
							<option value="all"><?=self::GetMessage('USE_FOR_ALL',true);?></option>
						</select>
					</div>
				<?endif?>
				<?
	...
	static function Process($ElementID, $arElement, $Params) {
		...
		$strUseFor = $Params['use_for'];
		$bCanProcess = $Params['is_offer'] || $strUseFor===null || in_array($strUseFor, array('product', 'all')); // обрабатываем все ТП, а товары - только если недоступна галочка применимости, или она установлена в product или all
		if (strlen($Target) && $bCanProcess) {
			...
			if(in_array($Target,array('NAME','CODE','SORT','EXTERNAL_ID','XML_ID','ACTIVE','PREVIEW_TEXT','PREVIEW_TEXT_TYPE','DETAIL_TEXT','DETAIL_TEXT_TYPE','PREVIEW_PICTURE','DETAIL_PICTURE','DATE_ACTIVE_FROM','DATE_ACTIVE_TO','SHOW_COUNTER','TAGS','DATE_CREATE','CREATED_BY','TIMESTAMP_X','MODIFIED_BY'))) {
				...
				// Save 
				if($Params['keep_exists_multiple_values']=='Y' && $arProp['MULTIPLE']=='Y' && is_array($arProp['PROPERTY_VALUE_ID']) && !empty($arProp['PROPERTY_VALUE_ID'])) {
					if(!is_array($Value)) {
						$Value = array($Value);
					}
					$arValueTmp = array();
					foreach($arProp['PROPERTY_VALUE_ID'] as $key => $intPropertyValueID) {
						if($arProp['PROPERTY_TYPE']=='L') {
							$arValueTmp[$intPropertyValueID] = $arProp['VALUE_ENUM_ID'][$key];
						} else { // S, S:directory, N, E, G, and others ...
							$arValueTmp[$intPropertyValueID] = $arProp['VALUE'][$key];
						}
					}
					$intIndex = 0;
					foreach($Value as $ValueItem) {
						$strValue = is_array($ValueItem) && isset($ValueItem['VALUE']) ? $ValueItem['VALUE'] : $ValueItem;
						if(strlen($strValue) && !in_array($strValue,$arValueTmp)) {
							$arValueTmp['n'.$intIndex] = $ValueItem;
							$intIndex++;
						}
					}
					$Value = $arValueTmp;
		...
		if($strUseFor!==null && !$Params['is_offer']) {
			if(in_array($strUseFor, array('offers','all'))) {
				...
				if($arCatalog['OFFERS_IBLOCK_ID']) {
					...
					$resOffers = CIBlockElement::GetList(array('ID'=>'ASC'),$arFilter,false,false,array('ID'));
					while($arOffer = $resOffers->getNext(false,false)){
						$arOfferFields = CWDA::GetElementByID($arOffer['ID']);
						$OfferParams = array_merge($Params, array('is_offer'=>true));
