...
class CWDA_Logger extends CWDA_Plugin {
	...
	CONST FILE_NAME = '/bitrix/modules/webdebug.antirutin/!log.#';
	CONST CSV_SEPARATOR = ';';
	//
	...
	static function GetMessage($Code, $ConvertCharset=false) {
		$MESS = array(
			...
			'SPECIAL_FIELD_1C_CODE' => 'Код товара из 1С',
				'SPECIAL_FIELD_1C_CODE_HEADER' => 'Код товара из 1С',
				'SPECIAL_FIELD_1C_CODE_TRAIT' => 'Код',
			//
			'IBLOCK_FIELD_SECTION_NAME' => 'Название раздела',
		);
		$MESS = trim($MESS[$Code]);
	...
	static function AddHeadData() {
		...
		function WDA_<?=self::CODE?>_Fill(){
			...
			var FieldsAll = $('#wda_fields_all').html(Select.html());
			FieldsAll.find('option[value=""],option[value=ACTIVE_DATE],option[value=SECTION_GLOBAL_ACTIVE]').remove();
			FieldsAll.find('option[value="SORT"]').after('<option value="SECTION_NAME"><?=self::getMessage('IBLOCK_FIELD_SECTION_NAME', true)?></option>');
			//
			...
			case 'CSV':
				...
				if($_SESSION['WDA_FIRST'] || $GLOBALS['WDA_FIRST']) {
					...
					foreach($arFields as $Code){
						...
						} elseif($Code == 'SECTION_NAME'){
							$Line = self::getMessage('IBLOCK_FIELD_SECTION_NAME', true);
				...
				foreach($arFields as $Code){
					...
					if(in_array($Code,array('ID','NAME','CODE','XML_ID','SORT','PREVIEW_TEXT','PREVIEW_TEXT_TYPE','DETAIL_TEXT','DETAIL_TEXT_TYPE','DATE_ACTIVE_FROM','DATE_ACTIVE_TO','SHOW_COUNTER','TAGS','DATE_CREATE','CREATED_BY','TIMESTAMP_X','MODIFIED_BY'))) {
						...
						$Line = '';
						if(is_numeric($arElement['IBLOCK_SECTION_ID']) && $arElement['IBLOCK_SECTION_ID'] > 0){
							$arSectionFilter = [
								'IBLOCK_ID' => $arElement['IBLOCK_ID'],
								'ID' => $arElement['IBLOCK_SECTION_ID'],
							];
							$resSection = \CIBlockSection::getList([], $arSectionFilter, false, ['NAME']);
