	...
	static function Process($ElementID, $arElement, $Params) {
		...
		if ((strlen($SourceField) || $SourcePropertyID>0) && (strlen($TargetField) || $TargetPropertyID>0)) {
			...
			if (!empty($Value)) {
				if (strlen($TargetField)) {
					...
					if ($Value>0) {
						...
						if ($IBlockElement->Update($arElement['ID'],array($TargetField=>$Value))) {
							...
							$strError = '['.$IBlockElement->LAST_ERROR.'] ';
						}
					}
				...
				$strError = '[empty value] ';
			}
		}
		if(!$bResult){
			CWDA::Log('Copy image ERROR '.$strError.'for element #'.$arElement['ID'].' (from '.$Params['field_source'].' to '.$Params['field_target'].')', self::CODE);
		}
		return $bResult;
	}
}
