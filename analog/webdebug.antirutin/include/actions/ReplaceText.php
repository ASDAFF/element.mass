...
class CWDA_ReplaceText extends CWDA_Plugin {
	...
	static function AddHeadData() {
		...
		$('#wda_checkbox_use_regexp input[type=checkbox]').on('change',function(){
			if($(this).is(':checked')) {
	...
	static function Process($ElementID, $arElement, $Params) {
		...
		elseif(preg_match('#^SEO_([A-z0-9_]+)$#',$Target,$M)){
			$obIPropValues = new \Bitrix\Iblock\InheritedProperty\ElementValues($arElement['IBLOCK_ID'], $arElement['ID']);
			$Value = html_entity_decode($obIPropValues->GetValue($M[1]));
			#
			$bResult = true;
			if(strlen($Value)){
				$Value = self::Replace($From, $To, $Value, $bUseRegExp, $bCaseSensitive, $ElementID);
