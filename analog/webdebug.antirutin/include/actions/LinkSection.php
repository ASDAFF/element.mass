	...
	static function GetMessage($Code, $ConvertCharset=false) {
		$MESS = array(
			'SECTION_LINK_TYPE_TITLE' => 'Выберите тип операции:',
				...
				'LINK_TYPE_CHANGE_MAIN' => 'Сменить основной раздел',
		...
		<div id="wda_settings_<?=self::CODE?>">
			...
			<div>
				<select name="params[section_link_type]" id="wda_select_section_link_type">
					...
					<option value="change_main"><?=self::GetMessage('LINK_TYPE_CHANGE_MAIN');?></option>
		...
		if($NewSectionID>0) {
			...
			switch($Params['section_link_type']) {
				...
				case 'change_main':
					...
					$bResult = $IBlockElement->Update($ElementID,array(
						'IBLOCK_SECTION_ID' => $NewSectionID,
						'IBLOCK_SECTION' => $arSectionsID,
					));
					\Bitrix\Iblock\PropertyIndex\Manager::updateElementIndex($arElement['IBLOCK_ID'], $ElementID);
					break;
				case 'add':
					$arSectionsID = array();
					$resSections = CIBlockElement::GetElementGroups($ElementID, true, array('ID'));
