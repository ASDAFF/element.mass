...
class CWDA_ElementUpdate extends CWDA_Plugin {
	...
	static function Process($ElementID, $arElement, $Params) {
		if($Params['event_CCatalogProduct_Update']=='Y' && CModule::includeModule('catalog')) {
			if (!CCatalogProduct::Update($ElementID,array('ID' => $ElementID))) {
				if(is_object($GLOBALS['APPLICATION']->LAST_ERROR)){
					$arMessages = $GLOBALS['APPLICATION']->LAST_ERROR->getMessages();
					$strMessage = implode(', ', $arMessages);
					CWDA::Log('Update product error [ID='.$ElementID.']: '.$strMessage);
				} else {
					CWDA::Log('Update product error [ID='.$ElementID.']');
				}
				return false;
			}
		}
		if($Params['event_CIBlockElement_Update']=='Y') {
			...
			$bUpdated = $IBlockElement->Update($ElementID,array('ID' => $ElementID),false,false,false,false);
			unset($IBlockElement);
			if(!$bUpdated) {
				CWDA::Log('Update element error [ID='.$ElementID.']: '.$IBlockElement->LAST_ERROR);
				return false;
			}
		}
		return true;
