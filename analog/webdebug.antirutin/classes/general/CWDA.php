...
class CWDA {
	 ...
	 *	Debug function
	 */
	public static function P($arData, $Return=false) {
	 ...
	 *	Add message to log
	 */
	public static function Log($Message, $ActionCode=false) {
	 ...
	 */
	public static function ConvertCharset($Data, $From='UTF-8', $To='CP1251') {
		global $APPLICATION;
		if (is_array($Data)) {
	 ...
	 *	Get list of iblocks
	 */
	public static function GetIBlockList($GroupByType=true, $ShowInActive=false) {
		$arResult = array();
	 ...
	 */
	public static function GetComparisonTypes($PropertyType=false, $Operation=false) {
		$arResult = array();
		...
		$intMonth = date('m');
		$intWeek = date('d');
		$intTimeStampToday = mktime(0, 0, 0, $intMonth, $intDay, $intYear);
		$intTimeStampWeek = time() - date('H')*60*60 - date('i')*60 - date('s');
		$intTimeStampMonth = time() - date('H')*60*60 - date('i')*60 - date('s');
		$intTimeStampYear = time() - date('H')*60*60 - date('i')*60 - date('s');
		*/
		#
		$intDay = date('d');
		$intYear = date('Y');
		...
		$strSiteDatetimeFormat = CDatabase::DateFormatToPHP(FORMAT_DATETIME);
		$intMonthDays = date('t');
		$intTimeStampToday = mktime(0, 0, 0, $intMonth, $intDay, $intYear);
		$intTimeStampYesterday = mktime(0, 0, 0, $intMonth, $intDay, $intYear) - 24*60*60;
		$intTimeStampWeek = mktime(0, 0, 0, $intMonth, $intDay, $intYear) - ($intWeekDay-1)*24*60*60;
		$intTimeStampMonth = mktime(0, 0, 0, $intMonth, 1, $intYear);
		$intTimeStampYear = mktime(0, 0, 0, 1, 1, $intYear);
		#
		$arResult['S:DATETIME'] = array( // DATETIME
			...
			'ISS' => array('NAME' => GetMessage('WDA_ISS'), 'MASK_PARAM' => '!#', 'MASK_VALUE' => false, 'VALUE' => 'N', 'NULL' => 'Y'),
			'NISS' => array('NAME' => GetMessage('WDA_NISS'), 'MASK_PARAM' => '#', 'MASK_VALUE' => false, 'VALUE' => 'N', 'NULL' => 'Y'),
			'TODAY' => array('NAME' => GetMessage('WDA_DATE_TODAY'), 'MASK_PARAM' => array('>=#', '<=#'), 'MASK_VALUE' => array(
				date($strSiteDatetimeFormat, $intTimeStampToday),
				date($strSiteDatetimeFormat, $intTimeStampToday + 24*60*60 - 1),
			), 'VALUE' => 'N'),
			'YESTERDAY' => array('NAME' => GetMessage('WDA_DATE_YESTERDAY'), 'MASK_PARAM' => array('>=#', '<=#'), 'MASK_VALUE' => array(
				date($strSiteDatetimeFormat, $intTimeStampYesterday),
				date($strSiteDatetimeFormat, $intTimeStampYesterday + 24*60*60 - 1),
			), 'VALUE' => 'N'),
			'THIS_WEEK' => array('NAME' => GetMessage('WDA_DATE_THIS_WEEK'), 'MASK_PARAM' => array('>=#', '<=#'), 'MASK_VALUE' => array(
				date($strSiteDatetimeFormat, $intTimeStampWeek),
				date($strSiteDatetimeFormat, $intTimeStampWeek + 7*24*60*60 - 1),
			), 'VALUE' => 'N'),
			'THIS_MONTH' => array('NAME' => GetMessage('WDA_DATE_THIS_MONTH'), 'MASK_PARAM' => array('>=#', '<=#'), 'MASK_VALUE' => array(
				date($strSiteDatetimeFormat, $intTimeStampMonth),
				date($strSiteDatetimeFormat, $intTimeStampMonth + $intMonthDays*24*60*60 - 1),
			), 'VALUE' => 'N'),
			'THIS_YEAR' => array('NAME' => GetMessage('WDA_DATE_THIS_YEAR'), 'MASK_PARAM' => array('>=#', '<=#'), 'MASK_VALUE' => array(
				date($strSiteDatetimeFormat, $intTimeStampYear),
				date($strSiteDatetimeFormat, mktime(23, 59, 59, 12, 31, $intYear)),
			), 'VALUE' => 'N'),
		);
		...
		$arResult['O'] = array( // Has offers?
			'Y' => array('NAME' => GetMessage('WDA_Y'), 'MASK_PARAM' => 'ID', 'MASK_VALUE' => '', 'VALUE' => 'N'),
			'N' => array('NAME' => GetMessage('WDA_N'), 'MASK_PARAM' => '!ID', 'MASK_VALUE' => '', 'VALUE' => 'N'),
		);
		$arResultTmp = array();
	 ...
	 */
	public static function GetComparisonTypesJSON($PropertyType=false, $Operation=false) {
	...
	public static function GetFields() {
		...
		$arResultTmp[] = array('WDA_TYPE'=>'O', 						'WDA_CODE'=>'_HAS_OFFERS');
		foreach($arResultTmp as $Key => $arItem) {
			if($arItem['WDA_CODE']=='_HAS_OFFERS'){
				$Name = GetMessage('WDA_HAS_OFFERS');
				if(!self::IsUtf()) {
					$Name = self::ConvertCharset($Name, 'CP1251', 'UTF-8');
				}
			} else {
				$Name = self::GetFieldName($arItem['WDA_CODE']);
			}
			$arItem['WDA_NAME_FULL'] = ($bShowCode?'['.$arItem['WDA_CODE'].'] ':'').$Name;
			$arItem['WDA_NAME'] = ($bShowCode?$arItem['WDA_CODE']:'').$Name;
	...
	public static function GetProperties($IBlockID) {
		...
		if (CModule::IncludeModule('iblock')) {
			$bUTF = self::IsUtf();
			$resProperty = CIBlockProperty::GetList(array('NAME'=>'ASC','SOR'=>'ASC'),array('IBLOCK_ID'=>$IBlockID,'ACTIVE'=>'Y'));
	 ...
	 *	Get list of prices
	 */
	public static function GetPriceTypeList($IBlockID=false, $arSort=array()) {
		$arResult = array();
	 ...
	 *	Get catalog fields
	 */
	public static function GetCatalogFields($IBlockID) {
		$arResult = array();
		if (CModule::IncludeModule('catalog') && $IBlockID>0 && self::IsCatalog($IBlockID)) {
			...
			$arStores = self::GetStoresList();
			foreach($arStores as $arStore){
				$arResult[] = array(
					'WDA_TYPE' => 'N',
					'WDA_CODE' => 'CATALOG_STORE_AMOUNT_'.$arStore['WDA_ID'],
					'WDA_NAME_FULL' => GetMessage('WDA_CATALOG_STORE_AMOUNT',array('#NAME#'=>$arStore['WDA_NAME_FULL'])),
					'WDA_NAME' => GetMessage('WDA_CATALOG_STORE_AMOUNT',array('#NAME#'=>$arStore['WDA_NAME'])),
					'WDA_GROUP' => 'CATALOG'
				);
			}
			$bUTF = self::IsUtf();
	 ...
	 *	Get all fields
	 */
	public static function GetAllFields($IBlockID) {
		$arResult = array();
		if (CModule::IncludeModule('iblock') && $IBlockID>0) {
			// Get fields
	 ...
	 */
	public static function CollectFilter($Fields, $Equals, $Values){
		$arResult = array();
	...
	public static function GetFilterItem(&$Param, &$Value, $arComparison) {
		if (is_array($arComparison) && !empty($arComparison)) {
			...
			if(is_array($arComparison['MASK_PARAM']) && is_array($arComparison['MASK_VALUE'])){
				foreach($arComparison['MASK_PARAM'] as $index => $param){
					$arComparison['MASK_PARAM'][$index] = str_replace('#',$Param,$arComparison['MASK_PARAM'][$index]);
				}
				#
				if($arComparison['NULL']=='Y') {
					$arComparison['MASK_VALUE'] = false;
				} else {
					foreach($arComparison['MASK_VALUE'] as $index => $value){
						$arComparison['MASK_VALUE'][$index] = str_replace('#',$Value,$arComparison['MASK_VALUE'][$index]);
					}
				}
				$Param = $arComparison['MASK_PARAM'];
				$Value = $arComparison['MASK_VALUE'];
			}
			else {
	 ...
	 *	Build filter
	 */
	public static function BuildFilter($IBlockID, $Sections, $IncludeSubsections, $FilterParams, $Fields) {
		$arResult = array();
			...
			foreach($FilterParams as $arItem) {
				...
				foreach($Fields as $arField) {
					if ($arField['WDA_CODE']==$Param) {
						...
						if($Param=='_HAS_OFFERS' && \CModule::IncludeModule('catalog')){
							$arCatalog = \CCatalog::GetByID($IBlockID);
							if(is_array($arCatalog) && $arCatalog['OFFERS_IBLOCK_ID']){
								$Key = is_array($Key) ? reset($Key) : $Key;
								$arResult[] = array(
									$Key => \CIBlockElement::SubQuery('PROPERTY_'.$arCatalog['OFFERS_PROPERTY_ID'], array(
	 ...
	 */
	public static function GetPropertyEnums($IBlockID, $PropertyID) {
		$arResult = array();
		if (CModule::IncludeModule('iblock')) {
			$bUTF = self::IsUtf();
	 ...
	 */
	public static function GetFilterFieldsGroups() {
		$arResult = array(
	 ...
	 */
	public static function GetApproximately($Count) {
		$Count = IntVal($Count);
		if ($Count>=500000) {
	 ...
	 */
	public static function IsCatalog($IBlockID){
		if (CModule::IncludeModule('catalog')) {
			$arCatalog = CCatalog::GetByID($IBlockID);
	 ...
	 *	Get actions groups
	 */
	public static function GetActionsGroup() {
		$arResult = array(
			'GENERAL' => GetMessage('WDA_GROUP_GENERAL'),
			'CATALOG' => GetMessage('WDA_GROUP_CATALOG'),
	 ...
	 *	Get single action
	 */
	public static function GetAction($Code,$arActions=false) {
		if ($arActions===false) {
	...
	public static function GetElementByID($ElementID) {
		...
		$intMaxPricesCountRegular = 10;
		$bRegularCatalogGroup = count($arPrices) <= $intMaxPricesCountRegular;
		if($bRegularCatalogGroup) {
		...
		if($obElement = $resElement->GetNextElement()) {
			...
			if(!$bRegularCatalogGroup) {
				self::AddElementCatalogPrices($arResult);
			}
	...
	public static function AddElementCatalogPrices(&$arElement){
		$intProductID = $arElement['ID'];
		if($intProductID){
			$arProduct = CCatalogProduct::getByID($intProductID);
			#
			$arPrices = self::GetPriceTypeList(false, array('ID'=>'ASC'));
			$arVat = self::GetVatList();
			#
			$arProductPrices = array();
			$resPrices = CPrice::getList(array('ID' => 'ASC', 'QUANTITY_FROM' => 'ASC'), array('PRODUCT_ID' => $intProductID));
			while($arProductPrice = $resPrices->getNext()){
				if(!isset($arProductPrices[$arProductPrice['CATALOG_GROUP_ID']])) {
					$arProductPrices[$arProductPrice['CATALOG_GROUP_ID']] = $arProductPrice;
				}
			}
			$arMap = array(
				'CATALOG_PRICE_ID' => 'ID',
				...
				'CATALOG_QUANTITY_FROM' => 'QUANTITY_FROM',
				'CATALOG_QUANTITY_TO' => 'QUANTITY_TO',
				'CATALOG_GROUP_NAME' => 'CATALOG_GROUP_NAME',
				'CATALOG_CAN_ACCESS' => 'CAN_ACCESS',
				'CATALOG_CAN_BUY' => 'CAN_BUY',
				'CATALOG_EXTRA_ID' => '',
			);
			foreach($arPrices as $asPrice){
				$intPriceID = $asPrice['ID'];
				$arProductPrice = is_array($arProductPrices[$intPriceID]) ? $arProductPrices[$intPriceID] : array();
				foreach($arMap as $from => $to){
					$strKey = $from.'_'.$intPriceID;
					$strValue = isset($arProductPrice[$to]) ? $arProductPrice[$to] : '';
					$arElement[$strKey] = $strValue;
					$arElement['~'.$strKey] = $strValue;
				}
			}
			#
			/*
			$arExcludeProductKeys = array('ID', 'TIMESTAMP_X');
			foreach($arProduct as $key => $value){
				if(!in_array($key, $arExcludeProductKeys)){
					$strKey = 'CATALOG_'.$key;
					$arElement[$strKey] = $value;
					$arElement['~'.$strKey] = $value;
				}
			}
			if($arProduct['VAT_ID'] && is_array($arVat[$arProduct['VAT_ID']])){
				$arElement['CATALOG_VAT'] = $arVat[$arProduct['VAT_ID']]['RATE'];
				$arElement['~CATALOG_VAT'] = $arVat[$arProduct['VAT_ID']]['RATE'];
			}
			*/
		}
	...
	public static function IsProperty($Code) {
	...
	public static function IsPrice($Code) {
	...
	public static function IsCatalogField($Code) {
	...
	public static function GetPropertyFromArrayById($Properties, $PropertyID) {
		$arResult = array();
	 ...
	 */
	public static function SetProductPrice($ProductID, $PriceID, $PriceValue, $Currency='RUB', $ExtraID=false) {
		$bResult = false;
	 ...
	 */
	public static function SetProductStoreQuantity($ProductID, $StoreID, $Quantity) {
		$bResult = false;
	...
	public static function RoundEx($Value, $Exp) {
		$Pow = Pow(10,$Exp);
	...
	public static function GetVatList($Empty=true) {
		$arResult = array();
		if($Empty){
			$arResult[] = array('NAME'=>GetMessage('WDA_CATALOG_VAT_EMPTY'));
		}
		if (CModule::IncludeModule('catalog')) {
	...
	public static function GetMeasureRatio($ProductID) {
		...
		if (CModule::IncludeModule('catalog')) {
			...
			if($arRatio = $resRatio->GetNext()){
				$mResult = FloatVal($arRatio['RATIO']);
			}
			/*
			if ($arRatio = $resRatio->GetNext(false,false)) {
				CCatalogMeasureRatio::Update($arRatio['ID'],array('RATIO'=>$arCatalog['MEASURE_RATIO']));
			} else {
				CCatalogMeasureRatio::Add(array('PRODUCT_ID'=>$ProductID,'RATIO'=>$arCatalog['MEASURE_RATIO']));
			}
			*/
		}
		return $mResult;
	...
	public static function ShowHint($Text) {
	 ...
	 */
	public static function GetCronCommand($ProfileID) {
		$strResult = false;
