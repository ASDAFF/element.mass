...
$(document).delegate(hid, 'fieldload', function(e){
	$('div[data-role="field-value"] select', this).not('[data-no-style="Y"]').not('[data-select2-id]').each(function(){
		wdaSelect2(this);
	});
});
...
// Refresh field
$(document).delegate(hid+' [data-role="plugin-form_refresh"]', 'click', function(e){
	e.preventDefault();
	$(this).parent().find('select').trigger('change');
});

// Replace text, for prevent elements titles like a 'Section name &quot;1&quot;'
$(document).delegate(hid+' [data-role="section_link_value"]', 'change', function(e){
	var input = this;
	setTimeout(function(){
		var span = $(input).parent().find('[data-role="section_link_title"]'),
			text = span.text(),
